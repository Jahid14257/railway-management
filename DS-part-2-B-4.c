...
struct create_account{
    ...
    char nid_or_birth_certificate[30];
...
temp=head;
 ...
 while(temp!=NULL){
    if(strcmp(ary1 ,temp->station_1)==0 && strcmp(ary2 ,temp->station_2)==0){
        printf("****Train : %s****\nSeat/price : %s\nTime : %s\n\n",temp->train_name,temp->seat_class,temp->time);
    }
    temp=temp->next;
 }
 char ary_trin[30],ary_seat[30],phone[15];
 FILE* by_ticket;
 by_ticket=fopen("ticket.txt","a");
 printf("Select your train :");
 gets(ary_trin);
 printf("Seat standard :");
 gets(ary_seat);
 printf("Contact number :");
 gets(phone);
 fprintf(by_ticket,"Train :%s\nSeat standard : %s\nContact number : %s\n",ary_trin,ary_seat,phone);
 fclose(by_ticket);
 printf("Your request for ticket is successful.\nPlease complete your payment and insert your contact number as reference.\nBkash:01762574910/018436578193\nNagad:01734098465\n******Thank You******\n");
...
void create_account(){
    ...
    create_account:
        ...
        scanf("%s", &create_user_1.nid_or_birth_certificate);
        printf("\nPhone Number = +880");
        scanf("%s", &create_user_1.phone_number);
        fflush(stdin);
        printf("User Name = ");
